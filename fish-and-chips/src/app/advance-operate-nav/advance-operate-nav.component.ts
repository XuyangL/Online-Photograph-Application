...
@Component({
  selector: 'app-advance-operate-nav',
  templateUrl: './advance-operate-nav.component.html',
  styleUrls: ['./advance-operate-nav.component.scss']
})
export class AdvanceOperateNavComponent implements OnInit {
  @Input() childData: string;
  @Input() isShown:boolean;
   
  constructor(private router: Router,private imageservice:ImageService,private changeservice:ChangeService) { }

  account() {
    console.log(this.childData);
    this.router.navigate(['/advanceAccount', this.childData]);
  }

  goDetect() {
    this.router.navigate(['/aidetect', this.childData]);
  }
  ...
  // status=this.changeservice.getSHow();
  showStickers():void{
    if(this.isShown){
        this.isShown=false;
  ...
  download(): void {
    ...
    if (oCanvas === null) {
      alert("You Cannot DownLoad Without An Image!!")
      return;
    }
  ...
  invert(event): void {
    ...
    darkness(event):void{
      ...
      for (var i = 0; i < data.length; i += 4) {
        ...
        data[i]     /= this.previoustrans/100; // red
        data[i + 1] /= this.previoustrans/100; // green
        data[i + 2] /= this.previoustrans/100; // blue
      }
    ...
    back():void{
      ...
      if(changes[changes.length-1].barnumber!==0){
        if(changes[changes.length-2].barnumber!==0){
          this.transval=changes[changes.length-2].barnumber;
        }
        else{
          this.transval=100;
        }
